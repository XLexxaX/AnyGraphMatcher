...
def exec(graph1, graph2, ml_model):
    ...
    for trainset in CONFIGURATION.gold_mapping.prepared_trainsets:
        if train is None:
            train = trainset.loc[:, ~(trainset.columns.isin(['src_id','tgt_id']))]#pd.read_csv(trainset, index_col=['Unnamed: 0'])
        else:
            tmp_train = trainset.loc[:, ~(trainset.columns.isin(['src_id','tgt_id']))] #pd.read_csv(trainset, index_col=['Unnamed: 0'])
            train = train.append(tmp_train, ignore_index=True)

    train = train[['label','syntactic_diff']]
    ...
    for testset in CONFIGURATION.gold_mapping.prepared_testsets:
        test = testset.loc[:, ~(testset.columns.isin(['src_id','tgt_id']))]#pd.read_csv(testset, index_col=['Unnamed: 0'])
        test = test[['label','syntactic_diff']]
