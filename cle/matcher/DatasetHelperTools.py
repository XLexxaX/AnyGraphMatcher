...
from cle.graphdatatools.InvertedIndexToolbox import getNGrams, InvertedIndex
import editdistance
from cle.matcher import DatasetPostHelperTools
...
def compute_append_syntactic_similarity_score(graph1, graph2, df, method='levenshtein', properties = None, plus_differentiator=False):
    ...
    if method == 'levenshtein':
        ...
        for index, row in df.iterrows():
            ...
            df.loc[index, attname] = float(editdistance.eval(txt1, txt2))/float(max(len(txt1), len(txt2)))
        #print("         Complete.")
