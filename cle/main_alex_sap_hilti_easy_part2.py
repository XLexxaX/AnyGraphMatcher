...
from cle.matcher import FlatMatcher
...
def main_full_string():
    ...
    gold_mapping = InternalGoldStandard({'trainsets':
                                            ...
                                            'sap_hilti_data', 'sap_hilti_full_strings', 'final_trainset.csv')],
                                         'testsets': [os.path.join(package_directory, '..', 'data',
                                            'sap_hilti_data', 'sap_hilti_full_strings', 'possible_matches.csv')]
                                        })
    dim = 20
    model = XGBClassifier()
    ...
    #line_a = pipeline.append_step(TSNEInterface.interface, PipelineDataTuple(line_a), PipelineDataTuple(2))
    #line_b = pipeline.append_step(TSNEInterface.interface, PipelineDataTuple(line_b), PipelineDataTuple(2))
    #line_ab = pipeline.append_step(CategoriesVisualizer.interface, PipelineDataTuple(line_a, line_b), None)
