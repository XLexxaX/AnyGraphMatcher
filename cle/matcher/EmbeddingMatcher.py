...
from matcher import Matchdata_Saver, PredictionToXMLConverter
import sys
import os
...
def exec(graph1, graph2, ml_model):
    ...
    for trainset in CONFIGURATION.gold_mapping.prepared_trainsets:
        if train is None:
            train = trainset.loc[:, ~(trainset.columns.isin(['src_id','tgt_id', 'src_category', 'tgt_category']))]
        else:
            tmp_train = trainset.loc[:, ~(trainset.columns.isin(['src_id','tgt_id', 'src_category', 'tgt_category']))]
    ...
    model = model.fit(
        x_train[[col for col in x_train.columns if not col == 'syntactic_diff' and not col == 'plus_diff']],
        y_train)
    ...
    for testset in CONFIGURATION.gold_mapping.prepared_testsets:
        test = testset.loc[:, ~(testset.columns.isin(['src_id','tgt_id', 'src_category', 'tgt_category']))]
        ...
        prediction = model.predict(x_test1[[col for col in x_test1.columns if not col == 'syntactic_diff' and not col == 'plus_diff']])
        ...
        CONFIGURATION.log("\n" + str([col for col in x_test1.columns if not col == 'syntactic_diff' and not col == 'plus_diff']))
    ...
    '''import scipy
    from matcher.DatasetHelperTools import extend_features, get_schema_data_from_graph
