...
def load(traindata):
    #traindata ="C:\\Users\\allue\\oaei_track_cache\\tmpdata\\"
    ...
    traindata = traindata + "data\\FB15K\\"
    ...
    weight = torch.FloatTensor(e.loc[:, 2:].to_numpy())
    print(e.loc[:, 2:].shape)
    ...
    weight2 = torch.FloatTensor(e2.loc[:, 2:].to_numpy())
    print(e2.loc[:, 2:].shape)
    ...
    rundir=traindata
    shutil.copy(rundir+"entity2id.txt", rundir+"..\\outputData\\entity2id.txt")
    shutil.copy(rundir+"relation2id.txt", rundir+"..\\outputData\\relation2id.txt")
