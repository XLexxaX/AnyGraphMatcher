...
#a mapping is an iterable of (source, target, relation, confidence)
...
class AlignmentHandler(object):
    ...
    def start(self, name, attrs):
        if name == self.base + 'entity1':
            self.one_cell[0] = parse.unquote(attrs[self.rdf + 'resource'])
        elif name == self.base + 'entity2':
            self.one_cell[1] = parse.unquote(attrs[self.rdf + 'resource'])
