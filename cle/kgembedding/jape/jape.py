...
from cle.kgembedding.jape.loss import *
from cle.kgembedding.jape.embed_func import *

logger = logging.getLogger(__name__)


def generate_cle_input():
...
def se_pos(embed_size = 75, batch_size = 10000, num_epochs = 1000, print_validation = 10):
    ...
    with graph.as_default():
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for epoch in range(num_epochs):
                ...
                logger.info("{}/{}, relation_loss = {:.3f}, time = {:.3f} s".format(epoch, num_epochs, pos_loss, end - start))
                ...
                if epoch % print_validation == 0 or epoch == num_epochs - 1:
                    ...
                    if early_stop_flag1 < 0 and early_stop_flag2 < 0 and hits < 0:
                        logger.info('time:{:.3f} s'.format(time.time() - total_start_time))
                        exit()
