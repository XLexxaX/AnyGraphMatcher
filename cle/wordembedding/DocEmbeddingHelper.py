...
ps = PorterStemmer()
import gensim.models.doc2vec as w2v
    ...
    with open(CONFIGURATION.rundir + "w2v_training_material.csv", mode="w+", encoding=CONFIGURATION.encoding) as f:
        for sent in sents:
            ...
            if len(tmp) > 1:
                ...
                f.write("\n")
        f.write("<> <>\n")
    return gensim.models.doc2vec.TaggedLineDocument(CONFIGURATION.rundir + "w2v_training_material.csv")
...
#    for doc in gensim.models.doc2vec.TaggedLineDocument(datapath(CONFIGURATION.rundir + "w2v_training_material.csv")):
#        yield doc
